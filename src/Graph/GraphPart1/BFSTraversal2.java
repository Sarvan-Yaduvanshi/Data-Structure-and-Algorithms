package Graph.GraphPart1;
import java.util.*;

public class BFSTraversal2 {
    static class Edge {
        int src, dest, weight;

        public Edge(int src, int dest, int weight) {
            this.src = src;
            this.dest = dest;
            this.weight = weight;
        }
    }

    // Method to create the graph using adjacency lists
    public static void createGraph(ArrayList<Edge>[] graph) {
        for (int i = 0; i < graph.length; i++) {
            graph[i] = new ArrayList<>();
        }

        // Adding edges for an undirected graph
        addEdge(graph, 0, 1, 1);
        addEdge(graph, 0, 2, 1);
        addEdge(graph, 1, 3, 1);
        addEdge(graph, 2, 4, 1);
        addEdge(graph, 3, 4, 1);
        addEdge(graph, 3, 5, 1);
        addEdge(graph, 4, 5, 1);
        addEdge(graph, 5, 6, 1);
    }

    // Helper method to add an edge to the undirected graph
    public static void addEdge(ArrayList<Edge>[] graph, int src, int dest, int weight) {
        graph[src].add(new Edge(src, dest, weight));
        graph[dest].add(new Edge(dest, src, weight));  // For undirected graphs
    }

    public static void bfs(ArrayList<Edge>[] graph) {
        boolean[] visited = new boolean[graph.length];
        for (int i = 0; i < graph.length; i++) {
            if (!visited[i]) {
                bfsUtil(graph, visited);
            }
        }
    }

    // BFS traversal method
    public static void bfsUtil(ArrayList<Edge>[] graph, boolean[] visited) {
        Queue<Integer> queue = new LinkedList<>();
        queue.add(0);  // Start from node 0
        visited[0] = true;  // Mark node 0 as visited

        while (!queue.isEmpty()) {
            int curr = queue.poll();
            System.out.print(curr + " ");

            // Traverse all adjacent nodes
            for (Edge edge : graph[curr]) {
                if (!visited[edge.dest]) {
                    queue.add(edge.dest);
                    visited[edge.dest] = true;  // Mark as visited when added to the queue
                }
            }
        }
    }

    public static void main(String[] args) {
        /*
                    1 --------- 3
                   /            | \
                  0             |   5 --------- 6
                  \             | /
                   2 ---------- 4
         */

        int V = 7;
        ArrayList<Edge>[] graph = new ArrayList[V];
        createGraph(graph);

        // Perform BFS traversal starting from vertex 0
        System.out.println("BFS Traversal starting from vertex 0:");
        bfs(graph);
    }
}

/*
     Explanation:
        Time Complexity: The BFS implementation works in O(V + E) where V is the number of vertices
                         and E is the number of edges. Each vertex and edge is visited once.
        Space Complexity: The space complexity is O(V + E) due to the adjacency list representation
                          of the graph, which stores all vertices and edges.
 */
